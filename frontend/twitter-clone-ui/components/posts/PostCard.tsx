...
export default function PostCard({
  ...
  const handleCardClick = () => router.push(`/post/${id}`);
  const stop = (e: React.MouseEvent) => e.stopPropagation();

  return (
    <article
      onClick={handleCardClick}
      className="border-b border-zinc-200 dark:border-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-900/50 transition-colors cursor-pointer"
    >
      {repostedByUsername && (
      ...
      <div className="flex gap-3 px-4 py-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div
            onClick={stop}
            className="flex items-center justify-between mt-3 text-zinc-500 max-w-[300px] -ml-2"
          >
            {/* Comment */}
            <button
              aria-label="Comment"
              onClick={handleCardClick}
              className="flex items-center gap-1.5 p-2 rounded-full hover:bg-sky-100 hover:text-sky-500 dark:hover:bg-sky-900/30 dark:hover:text-sky-400 transition-colors"
            >
            ...
            >
              ...
              {optimisticRepostCount > 0 && <span className="text-xs">{optimisticRepostCount}</span>}
            </button>
